...
// poszTexture = textureLoader.load( "images/ABCD.jpg" );   // useful for debugging
// negyTexture = textureLoader.load( "images/ABCD.jpg" );   // useful for debugging
// negxTexture = textureLoader.load( "images/ABCD.jpg" );   // useful for debugging
// negzTexture = textureLoader.load( "images/ABCD.jpg" );   // useful for debugging
...
floorNormalTexture.magFilter = THREE.LinearFilter;
...
const geometry = new THREE.SphereBufferGeometry( 3, 20, 10 );
sphereB = new THREE.Mesh( geometry, envmapMaterial );
sphereB.position.set(6,8,-1);
scene.add( sphereB );

const count = geometry.attributes.position.count;

const positionClone = JSON.parse(
  JSON.stringify(geometry.attributes.position.array)
);
const normalsClone = JSON.parse(
...
function update() {
  ...
  for (let i = 0; i < count; i++) {
    const uX = geometry.attributes.uv.getX(i) * Math.PI * wave;
    const uY = geometry.attributes.uv.getY(i) * Math.PI * wave;
